        ...
        public struct ScriptHash {
            ...
            public struct Subscribe: SwiftFulcrum.RPC.JSONRPCResponseAdapter {
                ...
                public init(fromRPC jsonrpc: JSONRPC) throws {
                    ...
                    case .scripthashAndStatus(let pair):
                        throw SwiftFulcrum.RPC.Response.Result.Error.unexpectedFormat("Expected a status string; got scripthash and status array for ScriptHash.Subscribe: \(pair.description)")
                    }
                }
            }
            ...
            public struct SubscribeNotification: SwiftFulcrum.RPC.JSONRPCResponseAdapter {
                ...
                public init(fromRPC jsonrpc: JSONRPC) throws {
                    switch jsonrpc {
                    case .scripthashAndStatus(let pair):
                        guard let first = pair.first, let scripthash = first else { throw SwiftFulcrum.RPC.Response.Result.Error.missingField("subscriptionIdentifier") }
                        self.subscriptionIdentifier = scripthash
                        self.status = (pair.count > 1) ? pair[1] : nil
                    case .status(let statusString):
                        throw SwiftFulcrum.RPC.Response.Result.Error.unexpectedFormat("Expected scripthash and status pair; got single status: \(statusString)")
                    }
                }
            }
