    ...
    struct Server: Decodable, Sendable {
        ...
        private static func normalizeURL(from rawValue: String, codingPath: [CodingKey]) throws -> URL {
            ...
            guard let rawComponents = URLComponents(string: trimmedRawValue) else {
                throw decodeError("Unable to parse server URL string.", codingPath: codingPath)
            }
            ...
            guard let result = components.url else {
                throw decodeError("Unable to construct server URL from components.", codingPath: codingPath)
            }
            guard let host = result.host,
                  !host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw decodeError("Host must not be empty.", codingPath: codingPath)
            }
        ...
        private static func normalizeURL(host: String, port: Int?, scheme: String?, codingPath: [CodingKey]) throws -> URL {
            ...
            guard let url = components.url else {
                throw decodeError("Unable to construct server URL from host and port.", codingPath: codingPath)
            }
        ...
        private static func validateNoUserInfo(in components: URLComponents, codingPath: [CodingKey]) throws {
            guard components.user == nil, components.password == nil else {
                throw decodeError("Server URL must not contain user info.", codingPath: codingPath)
            }
        }

        private static func validateNoFragment(in components: URLComponents, codingPath: [CodingKey]) throws {
            guard components.fragment == nil else {
                throw decodeError("Server URL must not contain a fragment.", codingPath: codingPath)
            }
        }
        ...
        private static func validatePort(_ port: Int?, codingPath: [CodingKey]) throws -> Int? {
            ...
            guard (1 ... 65_535).contains(port) else {
                throw decodeError("Port must be between 1 and 65535.", codingPath: codingPath)
            }
            return port
        }
        ...
        private static func normalizeScheme(from rawScheme: String?, codingPath: [CodingKey]) throws -> String {
            ...
            case "http":
                return "ws"
            default:
                throw decodeError("Unsupported server URL scheme.", codingPath: codingPath)
            }
        }
