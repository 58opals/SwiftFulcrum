...
actor WebSocketModel {
    ...
    private var connectionStateTracker: ConnectionStateTrackerModel
...
extension WebSocketModel {
    ...
    func makeDiagnosticsSnapshot() -> FulcrumClient.DiagnosticsModel.TransportSnapshotModel {
        .init(
...
// MARK: - Connect & Disconnect & ReconnectModel
extension WebSocketModel {
    func connect(
...
extension WebSocketModel {
    ...
    private func consumeQuietResponseIdentifier(for message: URLSessionWebSocketTask.Message) -> Bool {
        ...
        guard
            case .uuid(let identifier) = try? Response.JSONRPCModel.extractIdentifier(from: data),
