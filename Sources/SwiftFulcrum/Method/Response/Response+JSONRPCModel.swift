...
extension Response {
    public struct JSONRPCModel {
        ...
        public struct GenericModel<ResultModel: Decodable>: Decodable {
            ...
            let error: Response.Error.ResultModel?
            ...
            public init(from decoder: Decoder) throws {
                ...
                self.error = try container.decodeIfPresent(Response.Error.ResultModel.self, forKey: .error)
...
extension Response.JSONRPCModel: Sendable {}
extension Response.JSONRPCModel.GenericModel: Sendable where ResultModel: Sendable {}

extension Response.JSONRPCModel {
    static func extractIdentifier(from data: Data) throws -> Response.IdentifierModel {
        let response = try JSONRPCModel.CoderModel.decoder.decode(Response.JSONRPCModel.IdentifierExtractableModel.self, from: data)
        ...
        default:
            throw Response.JSONRPCModel.Error.wrongResponseType
        }
    }
}

extension Response.JSONRPCModel.GenericModel {
    func determineResponseType() throws -> Response.KindModel<ResultModel> {
        if let id,
           error == nil,
           method == nil,
           result == nil,
           hasResult {
            if let nilProducer = Response.JSONRPCModel.ResultNilProducerModel.produceNilIfOptional(ResultModel.self) {
                return .regular(Response.RegularModel(id: id, result: nilProducer))
            }
        }
        ...
        case let (id?, result?, _, _, _):
            return .regular(Response.RegularModel(id: id, result: result))
        case let (_, _, _, method?, params?):
            return .subscription(Response.SubscriptionModel(methodPath: method, result: params))
        case let (id?, _, error?, _, _):
            return .error(Response.Error(id: id, error: error))
        case let (id?, .none, _, _, _):
            return .empty(id)
        default:
            throw Response.JSONRPCModel.Error.cannotIdentifyResponseType(id)
        }
    }
}
...
extension Optional: Response.JSONRPCModel.NilConstructibleModel {
