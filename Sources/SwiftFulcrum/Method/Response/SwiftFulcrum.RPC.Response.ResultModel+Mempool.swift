...
extension SwiftFulcrum.RPC.Response.ResultModel {
    public struct Mempool {
        public struct GetInfo: SwiftFulcrum.RPC.ResponseProtocol {
            ...
            public typealias JSONRPCModel = SwiftFulcrum.RPC.Response.JSONRPCModel.Result.Mempool.GetInfo
        ...
        public struct GetFeeHistogram: SwiftFulcrum.RPC.ResponseProtocol {
            ...
            public struct Result: Decodable, Sendable {
                ...
                init(from pair: SwiftFulcrum.RPC.Response.JSONRPCModel.Result.Mempool.FeeHistogram) throws {
                    guard pair.count == 2 else { throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("Histogram entry must be [fee, vsize]; got \(pair)") }
                    let feeValue = pair[0].value
                    let virtualSizeValue = pair[1].value
                    guard feeValue.isFinite, feeValue >= 0 else { throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("Invalid fee: \(feeValue)") }
                    guard virtualSizeValue.isFinite, virtualSizeValue >= 0, virtualSizeValue <= Double(UInt.max) else { throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("Invalid vsize: \(virtualSizeValue)") }
            ...
            public typealias JSONRPCModel = SwiftFulcrum.RPC.Response.JSONRPCModel.Result.Mempool.GetFeeHistogram
            public init(fromRPC jsonrpc: JSONRPCModel) throws {
                self.histogram = try jsonrpc.enumerated().map { index, pair in
                    ...
                    catch { throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("Malformed entry at index \(index): \(error)") }
                }.sorted { $0.fee < $1.fee }
            }
        }
    }
