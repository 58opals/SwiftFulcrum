...
extension SwiftFulcrum.RPC.Response.ResultModel.Blockchain.Transaction {
            public struct Broadcast: SwiftFulcrum.RPC.ResponseProtocol {
                ...
                public typealias JSONRPCModel = SwiftFulcrum.RPC.Response.JSONRPCModel.Result.Blockchain.Transaction.Broadcast
                ...
                private static func decodeHex(_ hex: String) throws -> Data {
                    ...
                    guard string.count % 2 == 0 else {
                        throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("txid has odd hex length: \(string.count)")
                    }
                    ...
                    while index < string.endIndex {
                        ...
                        guard let byte = UInt8(byteString, radix: 16) else {
                            throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("tx contains non-hex: \(byteString)")
                        }
                        data.append(byte)
                        index = currentIndex
                    }
                    guard data.count == 32 else {
                        throw SwiftFulcrum.RPC.Response.ResultModel.Error.unexpectedFormat("txid decoded \(data.count) bytes; expected 32")
                    }
                    return data
                }
            }
