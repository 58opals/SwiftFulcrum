// SwiftFulcrum.ServerCatalog.Repository.swift
...
public extension SwiftFulcrum.ServerCatalog {
    struct Repository: Sendable {
        ...
        private let loadCatalog: @Sendable (SwiftFulcrum.Client.Configuration.NetworkModel, [URL]) async throws -> [URL]

        public init(load: @escaping @Sendable (SwiftFulcrum.Client.Configuration.NetworkModel, [URL]) async throws -> [URL]) {
            self.init(load: load, kind: .custom)
        }

        init(load: @escaping @Sendable (SwiftFulcrum.Client.Configuration.NetworkModel, [URL]) async throws -> [URL], kind: KindModel) {
            self.loadCatalog = load
            self.kind = kind
        }

        public func loadServers(
            for network: SwiftFulcrum.Client.Configuration.NetworkModel,
            fallback: [URL]
        ) async throws -> [URL] {
