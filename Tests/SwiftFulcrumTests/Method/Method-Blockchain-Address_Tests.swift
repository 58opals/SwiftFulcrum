        ...
        let balance = try await withRunningNode {
            let (_, result) = try await fulcrum.submit(
                ...
                responseType: Response.JSONRPC.Generic<
                    Response.JSONRPC.Result.Blockchain.Address.GetBalance>.self)
            return result
        }
    ...
    func getFirstUse() async throws {
        let first = try await withRunningNode {
            let (_, result) = try await fulcrum.submit(
                ...
                responseType: Response.JSONRPC.Generic<
                    Response.JSONRPC.Result.Blockchain.Address.GetFirstUse>.self)
            return result
        }
    ...
    func getHistory() async throws {
        let history = try await withRunningNode {
            let (_, items) = try await fulcrum.submit(
                ...
                responseType: Response.JSONRPC.Generic<
                    Response.JSONRPC.Result.Blockchain.Address.GetHistory>.self)
            return items
        }
    ...
    func getMempool() async throws {
        let mempool = try await withRunningNode {
            let (_, items) = try await fulcrum.submit(
                ...
                responseType: Response.JSONRPC.Generic<
                    Response.JSONRPC.Result.Blockchain.Address.GetMempool>.self)
            return items
        }
    ...
    func getScriptHash() async throws {
        let hash = try await withRunningNode {
            let (_, result) = try await fulcrum.submit(
                ...
                responseType: Response.JSONRPC.Generic<
                    Response.JSONRPC.Result.Blockchain.Address.GetScriptHash>.self)
            return result
        }
